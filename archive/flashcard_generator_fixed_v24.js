...
async function generatePDF(items, isFront, outputPath) {
    ...
    try {
        ...
        // Debug: Show more of the HTML to see if styles are included
        console.log('🔍 HTML Preview (first 1000 chars to see styles):');
        console.log(htmlContent.substring(0, 1000));
        console.log('🔍 Looking for </style> tag:');
        const styleEndIndex = htmlContent.indexOf('</style>');
        if (styleEndIndex !== -1) {
            console.log('✅ Found </style> at position:', styleEndIndex);
            console.log('🔍 Content around </style>:', htmlContent.substring(styleEndIndex - 100, styleEndIndex + 20));
        } else {
            console.log('❌ </style> tag NOT FOUND - CSS is not being included!');
        }
