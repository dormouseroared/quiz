...
(function() {
    ...
    <div id="mini-console">
      ...
      <div class="console-output" id="mini-console-output">
        <div class="console-line info">
          ...
          <span class="console-message">Console ready v24</span>
        </div>
      </div>
    </div>
  `;
  ...
  function setupControls() {
    ...
    // Clear unread when clicking on console output
    const output = document.getElementById('mini-console-output');
  ...
  function incrementUnreadCount() {
    ...
    // Only increment if console is minimized
    if (miniConsole.classList.contains('minimized')) {
      window.miniConsoleUnreadCount++;
      const badge = miniConsole.querySelector('.console-badge');
      badge.textContent = window.miniConsoleUnreadCount > 99 ? '99+' : window.miniConsoleUnreadCount;
      miniConsole.classList.add('has-unread');
    }
  }

  // Flash header for important messages
  ...
  // Regex pattern for escaping special characters
  ...
  function applyFilters(filterType, searchText) {
    ...
    lines.forEach(line => {
      ...
      if (searchText && searchMatch) {
        const escapedSearch = searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\      // Highlight search terms
      if (searchText && searchMatch) {
  ...
  function logToMini(message, type = 'log') {
    ...
    setTimeout(() => line.classList.remove('new-message'), 1000);
    ...
    // Increment unread counter and flash for errors/warnings
    if (type === 'error' || type === 'warn') {
      incrementUnreadCount();
      flashHeader();
    } else {
      // Still count regular logs when minimized
      incrementUnreadCount();
    }
    ...
    if (filterSelect && searchInput) {
      ...
      if (filterType !== 'all' && !line.classList.contains(filterType)) {
        ...
        if (!formattedMessage.toLowerCase().includes(searchLower)) {
          ...
          const escapedSearch = searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\  // Logging function
  function logToMini(message, type = 'log') {
    const output = document.getElementById('mini-console-output');
    if (!output) return;
    ...
    if (filterSelect && searchInput) {
      ...
      if (filterType !== 'all' && !line.classList.contains(filterType)) {
        ...
        if (!formattedMessage.toLowerCase().includes(searchLower)) {
          ...
          const escapedSearch = searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\        if (!formattedMessage.toLowerCase().includes(searchLower)) {
          line.style.display = 'none';
        } else {
          const escapedSearch = searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          const regex = new RegExp(`(${escapedSearch})`, 'gi');
