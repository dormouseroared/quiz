...
function generateCSS() {
    ...
    <style>
        ...
        ul {
            ...
            margin: 0;
            padding-left: 15px;
            font-size: 13px; /* Smaller for variable lists */
            line-height: 1.3;
        }
...
console.log("filter for flashcards found:", flashcardFullItems.length)

const flashcardItems = flashcardFullItems.filter(item =>
    (item.flashcards.front && item.flashcards.front.trim()) || (item.flashcards.back && item.flashcards.back.trim()) ||
...
pages.forEach((pageCards, pageIndex) => {
    ...
    pageCards.forEach(item => {
        const frontContent = item.flashcards.front || 'Question not defined'
        questionsHTML += `
            <div class="card">
...
pages.forEach((pageCards, pageIndex) => {
    ...
    pageCards.forEach(item => {
        const backContent = item.flashcards.back || item.text || 'Answer not defined'
        answersHTML += `
            <div class="card">
                <div class="card-header">
                    ${item.key} | ${item.title}
