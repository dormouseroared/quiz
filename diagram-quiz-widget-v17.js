...
export default function diagramQuizWidget(quizCards, targetDiv) {
  ...
  function render(card) {
      ...
      <div style="background: white; border-radius: 12px; max-width: 1400px; width: 95%; max-height: 95vh; overflow: auto; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.3);">
        <div style="padding: 24px;">
          ...
          <div style="display: flex; gap: 8px; margin-bottom: 16px;">
            ...
            <button id="next-btn" style="padding: 8px 16px; background: #16a34a; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 500; ${currentCardIndex >= quizCards.length - 1 ? 'display: none;' : ''}">
              Next Card →
            </button>
    ...
    console.log('Next button:', nextBtn, 'Current index:', currentCardIndex, 'Total cards:', quizCards.length)
    if (nextBtn) {
      console.log('Next button found, adding listener')
      console.log('Button computed style:', window.getComputedStyle(nextBtn).pointerEvents)
      console.log('Button display:', window.getComputedStyle(nextBtn).display)
      
      nextBtn.addEventListener('mousedown', () => {
        console.log('MOUSEDOWN detected on next button')
      })
      ...
      nextBtn.addEventListener('click', (e) => {
        ...
        if (currentCardIndex < quizCards.length - 1) {
          currentCardIndex++
          console.log('Moving to card:', currentCardIndex)
          loadCard(currentCardIndex)
        } else {
