...
class VideoNavigator {
    ...
    setupEventListeners() {
        ...
        // Video selection
        this.videoSelect.addEventListener('change', (e) => this.loadVideo(e.target.value))
    ...
    populateVideoSelector() {
        ...
        this.videoSelect.innerHTML = '<option value="">-- Choose a video --</option>'
    ...
    displayClipsForVideo(videoId) {
        ...
        if (clips.length === 0) {
            this.clipsList.innerHTML = '<p style="color: #999;">No clips defined for this video yet</p>'
            return
        }
    ...
    handleKeyboard(e) {
        ...
        if (e.code === 'ArrowLeft') {
            ...
            this.videoPlayer.currentTime = Math.max(0, this.videoPlayer.currentTime - skipAmount)
        ...
        if (e.code === 'ArrowUp') {
            ...
            this.videoPlayer.playbackRate = Math.min(2.0, this.videoPlayer.playbackRate + 0.1)
            // Round to avoid floating point weirdness (0.99999999)
            this.videoPlayer.playbackRate = Math.round(this.videoPlayer.playbackRate * 10) / 10
        ...
        if (e.code === 'ArrowDown') {
            ...
            this.videoPlayer.playbackRate = Math.max(0.5, this.videoPlayer.playbackRate - 0.1)
